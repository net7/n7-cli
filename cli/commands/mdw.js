...
const placeholdersToReplace = {
  ...
  from: [/basemdwname/g, /BASEMDWNAME/g, /Basemdwname/g],
  files: [
    'package.json',
    'README.md',
    'src/parsers/basemdwname-home.ts',
...
class CommandMdw {
  constructor(name, verbose) {
    ...
    this.targetPath = `${process.cwd()}/${this.name}-mdw`;
    this.srcPath = path.join(path.dirname(fs.realpathSync(__filename)), '../../base-mdw');
    ...
    this.directoryExists()
      ...
      .then(() => {
        helpers.log(`new muruca mdw project created! path: ${this.targetPath}`);
      })
      .catch(reason => {
  ...
  replaceFiles() {
    ...
    filesToReplace.forEach((file) => {
      const src = file;
      const dest = file.replace('basemdwname', this.name);
      files.push({ src, dest })
    });
    ...
    this.printInfo([
      'replacing files',
      ...files.map(({ src, dest }) => `- ${this.name}-mdw/${src} => ${this.name}-mdw/${dest}`)
    ].join('\n'));
  ...
  cleanUpFiles() {
    ...
    this.printInfo([
      ...
      ...files.map((file) => `- ${this.name}-mdw/${file}`)
    ].join('\n'));
  ...
  replacePlaceholders() {
    ...
    files = files.map((file) => file.replace('basemdwname', this.name));
